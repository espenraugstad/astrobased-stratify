...
export default function MergeProgressDialog({ sources, target, setMergePhase, setSourceIds, setTargetId }: MergeProgressDialogProps) {
    ...
    return (
        <dialog ref={dialogRef} className="flex flex-col items-center min-w-1/2 m-auto backdrop:bg-black backdrop:opacity-70 p-4">
            ...
            <button disabled={!mergeComplete} className="my-8 disabled:bg-slate-300 disabled:cursor-not-allowed lm-4 w-48 h-14 text-sm text-white font-bold rounded-full bg-green-900 hover:bg-green-700 hover:cursor-pointer" onClick={closeDialog}>Done</button>
        </dialog>)
}